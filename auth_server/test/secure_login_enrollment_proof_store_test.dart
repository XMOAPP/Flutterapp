...
void main() {
  ...
  test('proof is bound to its verified email and can be used once', () {
    ...
    expect(store.claim(proof: proof, email: 'other@example.com'), isNull);
  ...
  test('pending proof can be checked without consuming it', () {
    final proof = store.issue('person@example.com');

    expect(
      store.isPendingForEmail(proof: proof, email: 'person@example.com'),
      isTrue,
    );
    expect(
      store.isPendingForEmail(proof: proof, email: 'other@example.com'),
      isFalse,
    );
    expect(store.claim(proof: proof, email: 'person@example.com'), isNotNull);
  });
  ...
  test('expired proof cannot be claimed or restored', () {
    ...
    expect(store.claim(proof: proof, email: 'person@example.com'), isNull);
    expect(store.length, 0);
  });

  test('claimed proof can be restored after a transient provider failure', () {
    final proof = store.issue('person@example.com');
    final claim = store.claim(proof: proof, email: 'person@example.com');
  ...
  test('proof can resume after process stops while claim is in flight', () {
    ...
    final proof = first.issue('person@example.com');
    expect(first.claim(proof: proof, email: 'person@example.com'), isNotNull);
  ...
  test('completed proof survives a server restart', () {
    ...
    final proof = first.issue('person@example.com');
    final claim = first.claim(proof: proof, email: 'person@example.com')!;
